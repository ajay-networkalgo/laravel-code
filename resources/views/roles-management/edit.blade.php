      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card card-primary">
            ...
            <form action="{{ route('roles.update') }}" method="post" id="addUserForm">
              <input type="hidden" name="role_id" id="role_id" value="{{$user_role->id}}">
              <input type="hidden" name="_token" value="{{ csrf_token() }}" />
              <div class="card-body">
                <div class="form-group row">
                  <div class="col-md-6">
                    <label>Name<span style="color: red;">*</span></label>
                    <input type="text" class="form-control" name="name" id="name" placeholder="Enter name" value="{{ old('name', $user_role->roles) }}">
                    @error('name')
                ...
                <div class="form-group">
                    @foreach ($module_lists as $module)
                      <div class="module-permission-item" style="margin-bottom: 10px;">
                        <label>
                            <input type="checkbox" class="module-checkbox" name="modules[]" value="{{ $module->id }}" {{ in_array($module->id, $selected_modules) ? 'checked' : '' }}>
                            {{ $module->title }}
                        </label>
                    </div>
                    @endforeach
