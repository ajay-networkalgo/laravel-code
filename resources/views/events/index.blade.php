        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              ...
              <a href="{{ route('events.add') }}" class="btn btn-primary" style="float:right;">Add events</a>
            </div>
            <div class="card-body">
              <table id="events_list" class="table table-bordered">
                <thead>
                  <tr>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#events_list').DataTable().on('click', '.deleteRecord', function (e) { 
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "DELETE",
            url: siteUrl+"admin/events/delete/"+eventId,
            data: {
                _token: '{{ csrf_token() }}'
            },
            success: function (response) {
                if (response.status == true) {
                  ...
                  $('#events_list').DataTable().ajax.reload();
                }
            },
    ...
    $('#events_list').DataTable().on('click', '.changeStatus', function (e) {
      var id = $(this).attr('id');
      ...
      .then((result) => {
        if (result.isConfirmed) {
          $.ajax({
            url: siteUrl+"admin/events/changestatus",
            type: "post",
            data: {
